...
def find_coverage_by_large_sub_rectangles(coords: Coords) -> list[Rectangle]:
    ...
    if not coverage == coords:
        raise ValueError(
            "The mask is not entirely covered by rectangles, something went wrong."
        )
...
def find_sub_rectangles(coords: Coords, overlapping=True) -> list[Rectangle]:
    ...
    for start_col in range(col_min, col_max + 1):
        for start_row in range(row_min, row_max + 1):
            ...
            # Expand the rectangle to the right:
            while (
                start_col + width <= col_max
                and (start_col + width, start_row) in coords
            ):
            ...
            # Expand the rectangle downwards:
            while start_row + height <= row_max and all(
                (start_col + dcol, start_row + height) in coords
                for dcol in range(width)
            ):
                if not overlapping and any(
                    (start_col + dcol, start_row + height) in seen
                    for dcol in range(width)
                ):
                    break
                height += 1
            ...
            if (
                width >= DIMENSION_SIZE_REQUIREMENT
...
def filter_and_order_rectangles_deprectaed(
    rectangles: list[Rectangle],
) -> list[Rectangle]:
    ...
    return [
        ...
        if not any(is_rectangle_included(rect, prect) for prect in rectangles[:i])
    ]
...
def select_rectangles(coords: Coords, rectangles: list[Rectangle]) -> list[Rectangle]:
    ...
    coverage: Dict[Coord, int] = defaultdict(
        int
    )  # Number of time a coords is covered. keys=(col, rw), value=(count)
    ...
    for rect in rectangles:
        ...
        if all(
            coverage[coord] < MAX_OVERLAP or coord not in coords
...
def splice_by_removing_a_rectangle(
...
@debug
def condition_by_rectangles_deprecated(
    coords: Coords,
) -> tuple[list[Coords], OrthogonalSet]:
