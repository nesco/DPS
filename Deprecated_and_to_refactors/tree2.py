...
def next_layer(layer: list[Node]) -> list[Node]:
    return [
        child for node in layer if isinstance(node, RoseTree) for child in node.children
    ]
...
def map_node(tree: Node, f: Callable[[T | U], Any]) -> Node:
    ...
    match tree:
        ...
        case RoseTree(value, children):
            return RoseTree(
                value=f(value), children=[map_node(child, f) for child in children]
            )
...
def map_leaf(tree: Node, f: Callable[[U], Any]) -> Node:
    ...
    match tree:
        ...
        case RoseTree(value, children):
            return RoseTree(
                value=value, children=[map_node(child, f) for child in children]
            )
...
def test_traversal_functions():
    ...
    assert values_bfs(tree1) == expected_bfs_tree1, (
        f"values_bfs failed for tree1. Expected {expected_bfs_tree1}, got {values_bfs(tree1)}"
    )
    assert values_dfs(tree1) == expected_dfs_tree1, (
        f"values_dfs failed for tree1. Expected {expected_dfs_tree1}, got {values_dfs(tree1)}"
    )
    ...
    assert values_bfs(tree2) == expected_bfs_tree2, (
        f"values_bfs failed for tree2. Expected {expected_bfs_tree2}, got {values_bfs(tree2)}"
    )
    assert values_dfs(tree2) == expected_dfs_tree2, (
        f"values_dfs failed for tree2. Expected {expected_dfs_tree2}, got {values_dfs(tree2)}"
    )
    ...
    tree3 = RoseTree(
        ...
        [
            ...
            Leaf(5),
            RoseTree(6, [RoseTree(7, [Leaf(8)]), Leaf(9)]),
        ],
    )
    # Expected BFS order:
    # Level 0: 1
    ...
    assert values_bfs(tree3) == expected_bfs_tree3, (
        f"values_bfs failed for tree3. Expected {expected_bfs_tree3}, got {values_bfs(tree3)}"
    )
    assert values_dfs(tree3) == expected_dfs_tree3, (
        f"values_dfs failed for tree3. Expected {expected_dfs_tree3}, got {values_dfs(tree3)}"
    )
...
if __name__ == "__main__":
    ...
    tree4 = RoseTree(
        "root", [Leaf("leaf1"), RoseTree("branch1", [Leaf("leaf2")]), Leaf("leaf3")]
    )
    ...
    tree9 = RoseTree(1, [RoseTree(2, [Leaf(3)]), RoseTree(4, [RoseTree(5, [Leaf(6)])])])
