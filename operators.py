...
def fill_binary(
    mask1: Mask, mask2: Mask, condition: Callable[[bool, bool], bool]
) -> Mask:
...
def morphological_opening(mask_object: Mask, mask_kernel: Mask):
    return morphological_dilatation(
        morphological_erosion(mask_object, mask_kernel), mask_kernel
    )


def morphological_closing(mask_object: Mask, mask_kernel: Mask):
    return morphological_erosion(
        morphological_dilatation(mask_object, mask_kernel), mask_kernel
    )
...
def condition_by_connected_components(
    coords: Coords, limits: Optional[Box] = None, chebyshev: bool = True
) -> list[Coords]:
    """
...
def partition(mask_connected_component, chebyshev=False):
    ...
    # and vice-versa
    component_number, distribution, components = connected_components(
        mask_complement, chebyshev=not chebyshev
    )
    components_partition = {}
    for key in components.keys():
        components_partition[key] = {
            "mask": components[key]["mask"],
...
def euler_characteristic(components_partition):
    ...
    nb_holes = sum(
        [
            0 if component["is_exterior"] == 1 else 1
            for component in components_partition.values()
        ]
    )
    return 1 - nb_holes
...
def topological_contains(mask1, mask2, chebyshev=False):
    ...
    while (
        set_contains(interior, mask_growing) and cardinal(mask_growing) < height * width
    ):
        dilatation_number += 1
...
def _test_intersection():
    assert intersection(
        binary_to_bool_grid([[1, 1], [0, 1]]), binary_to_bool_grid([[1, 0], [1, 1]])
    ) == binary_to_bool_grid([[1, 0], [0, 1]])
    assert intersection(
        binary_to_bool_grid([[1, 0], [0, 1]]), binary_to_bool_grid([[0, 1], [1, 0]])
    ) == binary_to_bool_grid([[0, 0], [0, 0]])
...
def _test_union():
    ...
    assert union(
        binary_to_bool_grid([[1, 0], [0, 0]]), binary_to_bool_grid([[0, 0], [0, 1]])
    ) == binary_to_bool_grid([[1, 0], [0, 1]])
